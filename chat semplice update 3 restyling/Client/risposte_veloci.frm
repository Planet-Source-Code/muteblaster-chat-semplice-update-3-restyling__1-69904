VERSION 5.00
Begin VB.Form risposte_veloci 
   BackColor       =   &H80000013&
   BorderStyle     =   0  'None
   ClientHeight    =   8685
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   6045
   LinkTopic       =   "Form1"
   ScaleHeight     =   8685
   ScaleWidth      =   6045
   ShowInTaskbar   =   0   'False
   StartUpPosition =   3  'Windows Default
   Begin client.CandyButton CandyButton8 
      Height          =   375
      Left            =   4800
      TabIndex        =   18
      Top             =   7200
      Width           =   975
      _ExtentX        =   1720
      _ExtentY        =   661
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "Tahoma"
         Size            =   9
         Charset         =   0
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Caption         =   "inserisci"
      IconHighLiteColor=   0
      CaptionHighLiteColor=   0
      Style           =   6
      Checked         =   0   'False
      ColorButtonHover=   16760976
      ColorButtonUp   =   15309136
      ColorButtonDown =   15309136
      BorderBrightness=   0
      ColorBright     =   16772528
      DisplayHand     =   0   'False
      ColorScheme     =   0
   End
   Begin client.CandyButton CandyButton7 
      Height          =   375
      Left            =   4800
      TabIndex        =   17
      Top             =   6360
      Width           =   975
      _ExtentX        =   1720
      _ExtentY        =   661
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "Tahoma"
         Size            =   9
         Charset         =   0
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Caption         =   "inserisci"
      IconHighLiteColor=   0
      CaptionHighLiteColor=   0
      Style           =   6
      Checked         =   0   'False
      ColorButtonHover=   16760976
      ColorButtonUp   =   15309136
      ColorButtonDown =   15309136
      BorderBrightness=   0
      ColorBright     =   16772528
      DisplayHand     =   0   'False
      ColorScheme     =   0
   End
   Begin client.CandyButton CandyButton6 
      Height          =   375
      Left            =   4800
      TabIndex        =   16
      Top             =   5520
      Width           =   975
      _ExtentX        =   1720
      _ExtentY        =   661
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "Tahoma"
         Size            =   9
         Charset         =   0
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Caption         =   "inserisci"
      IconHighLiteColor=   0
      CaptionHighLiteColor=   0
      Style           =   6
      Checked         =   0   'False
      ColorButtonHover=   16760976
      ColorButtonUp   =   15309136
      ColorButtonDown =   15309136
      BorderBrightness=   0
      ColorBright     =   16772528
      DisplayHand     =   0   'False
      ColorScheme     =   0
   End
   Begin client.CandyButton CandyButton5 
      Height          =   375
      Left            =   4800
      TabIndex        =   15
      Top             =   4680
      Width           =   975
      _ExtentX        =   1720
      _ExtentY        =   661
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "Tahoma"
         Size            =   9
         Charset         =   0
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Caption         =   "inserisci"
      IconHighLiteColor=   0
      CaptionHighLiteColor=   0
      Style           =   6
      Checked         =   0   'False
      ColorButtonHover=   16760976
      ColorButtonUp   =   15309136
      ColorButtonDown =   15309136
      BorderBrightness=   0
      ColorBright     =   16772528
      DisplayHand     =   0   'False
      ColorScheme     =   0
   End
   Begin client.CandyButton CandyButton4 
      Height          =   375
      Left            =   4800
      TabIndex        =   14
      Top             =   3840
      Width           =   975
      _ExtentX        =   1720
      _ExtentY        =   661
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "Tahoma"
         Size            =   9
         Charset         =   0
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Caption         =   "inserisci"
      IconHighLiteColor=   0
      CaptionHighLiteColor=   0
      Style           =   6
      Checked         =   0   'False
      ColorButtonHover=   16760976
      ColorButtonUp   =   15309136
      ColorButtonDown =   15309136
      BorderBrightness=   0
      ColorBright     =   16772528
      DisplayHand     =   0   'False
      ColorScheme     =   0
   End
   Begin client.CandyButton CandyButton3 
      Height          =   375
      Left            =   4800
      TabIndex        =   13
      Top             =   3000
      Width           =   975
      _ExtentX        =   1720
      _ExtentY        =   661
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "Tahoma"
         Size            =   9
         Charset         =   0
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Caption         =   "inserisci"
      IconHighLiteColor=   0
      CaptionHighLiteColor=   0
      Style           =   6
      Checked         =   0   'False
      ColorButtonHover=   16760976
      ColorButtonUp   =   15309136
      ColorButtonDown =   15309136
      BorderBrightness=   0
      ColorBright     =   16772528
      DisplayHand     =   0   'False
      ColorScheme     =   0
   End
   Begin client.CandyButton CandyButton2 
      Height          =   375
      Left            =   4800
      TabIndex        =   12
      Top             =   2160
      Width           =   975
      _ExtentX        =   1720
      _ExtentY        =   661
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "Tahoma"
         Size            =   9
         Charset         =   0
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Caption         =   "inserisci"
      IconHighLiteColor=   0
      CaptionHighLiteColor=   0
      Style           =   6
      Checked         =   0   'False
      ColorButtonHover=   16760976
      ColorButtonUp   =   15309136
      ColorButtonDown =   15309136
      BorderBrightness=   0
      ColorBright     =   16772528
      DisplayHand     =   0   'False
      ColorScheme     =   0
   End
   Begin client.CandyButton CandyButton1 
      Height          =   375
      Left            =   4800
      TabIndex        =   11
      Top             =   1320
      Width           =   975
      _ExtentX        =   1720
      _ExtentY        =   661
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "Tahoma"
         Size            =   9
         Charset         =   0
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Caption         =   "inserisci"
      IconHighLiteColor=   0
      CaptionHighLiteColor=   0
      Style           =   6
      Checked         =   0   'False
      ColorButtonHover=   16760976
      ColorButtonUp   =   15309136
      ColorButtonDown =   15309136
      BorderBrightness=   0
      ColorBright     =   16772528
      DisplayHand     =   0   'False
      ColorScheme     =   0
   End
   Begin client.CandyButton Cmdsalva 
      Height          =   495
      Left            =   2880
      TabIndex        =   10
      Top             =   7920
      Width           =   1695
      _ExtentX        =   2990
      _ExtentY        =   873
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "Tahoma"
         Size            =   11.25
         Charset         =   0
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Caption         =   "salva"
      IconHighLiteColor=   0
      CaptionHighLiteColor=   0
      ForeColor       =   -2147483630
      Style           =   7
      Checked         =   0   'False
      ColorButtonHover=   16760976
      ColorButtonUp   =   15309136
      ColorButtonDown =   15309136
      BorderBrightness=   0
      ColorBright     =   16772528
      DisplayHand     =   0   'False
      ColorScheme     =   0
   End
   Begin VB.TextBox Text8 
      Height          =   615
      Left            =   480
      TabIndex        =   9
      Top             =   7080
      Width           =   4095
   End
   Begin VB.TextBox Text7 
      Height          =   615
      Left            =   480
      TabIndex        =   8
      Top             =   6240
      Width           =   4095
   End
   Begin VB.TextBox Text6 
      Height          =   615
      Left            =   480
      TabIndex        =   7
      Top             =   5400
      Width           =   4095
   End
   Begin VB.TextBox Text5 
      Height          =   615
      Left            =   480
      TabIndex        =   6
      Top             =   4560
      Width           =   4095
   End
   Begin VB.TextBox Text4 
      Height          =   615
      Left            =   480
      TabIndex        =   5
      Top             =   3720
      Width           =   4095
   End
   Begin VB.TextBox Text3 
      Height          =   615
      Left            =   480
      TabIndex        =   4
      Top             =   2880
      Width           =   4095
   End
   Begin VB.TextBox Text2 
      Height          =   615
      Left            =   480
      TabIndex        =   3
      Top             =   2040
      Width           =   4095
   End
   Begin VB.TextBox Text1 
      Height          =   615
      Left            =   480
      TabIndex        =   2
      Top             =   1200
      Width           =   4095
   End
   Begin client.chameleonButton chameleonButton1 
      Height          =   7815
      Left            =   120
      TabIndex        =   1
      Top             =   840
      Width           =   5775
      _ExtentX        =   10186
      _ExtentY        =   13785
      BTYPE           =   3
      TX              =   ""
      ENAB            =   -1  'True
      BeginProperty FONT {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      COLTYPE         =   1
      FOCUSR          =   -1  'True
      BCOL            =   14215660
      BCOLO           =   14215660
      FCOL            =   0
      FCOLO           =   0
      MCOL            =   12632256
      MPTR            =   1
      MICON           =   "risposte_veloci.frx":0000
      UMCOL           =   -1  'True
      SOFT            =   0   'False
      PICPOS          =   0
      NGREY           =   0   'False
      FX              =   0
      HAND            =   0   'False
      CHECK           =   0   'False
      VALUE           =   0   'False
   End
   Begin VB.Label Label1 
      BackStyle       =   0  'Transparent
      Caption         =   "scrivete le risoste rapide come promemoria da inviare al momento giusto"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   12
         Charset         =   0
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   735
      Left            =   240
      TabIndex        =   0
      Top             =   120
      Width           =   4935
   End
   Begin VB.Shape Shape1 
      Height          =   8655
      Left            =   0
      Top             =   0
      Width           =   6015
   End
End
Attribute VB_Name = "risposte_veloci"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Private Sub form_load()
 Text1 = RegLoad(Text1)
 Text2 = RegLoad(Text2)
 Text3 = RegLoad(Text3)
 Text4 = RegLoad(Text4)
 Text5 = RegLoad(Text5)
 Text6 = RegLoad(Text6)
 Text7 = RegLoad(Text7)
 Text8 = RegLoad(Text8)
End Sub

Private Sub SaveControlValues()
 Call RegSave(Text1, Text1.Text)
 Call RegSave(Text2, Text2.Text)
 Call RegSave(Text3, Text3.Text)
 Call RegSave(Text4, Text4.Text)
 Call RegSave(Text5, Text5.Text)
 Call RegSave(Text6, Text6.Text)
 Call RegSave(Text7, Text7.Text)
 Call RegSave(Text8, Text8.Text)
End Sub

Private Sub CandyButton1_Click()
 chat.txtsend.Text = Text1.Text
End Sub

Private Sub CandyButton2_Click()
 chat.txtsend.Text = Text2.Text
End Sub

Private Sub CandyButton3_Click()
 chat.txtsend.Text = Text3.Text
End Sub

Private Sub CandyButton4_Click()
 chat.txtsend.Text = Text4.Text
End Sub

Private Sub CandyButton5_Click()
 chat.txtsend.Text = Text5.Text
End Sub

Private Sub CandyButton6_Click()
 chat.txtsend.Text = Text6.Text
End Sub

Private Sub CandyButton7_Click()
 chat.txtsend.Text = Text7.Text
End Sub

Private Sub CandyButton8_Click()
 chat.txtsend.Text = Text8.Text
End Sub

Private Sub Cmdsalva_Click()
 Call SaveControlValues
End Sub
